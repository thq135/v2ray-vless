...
func TestRequestSerialization(t *testing.T) {
	...
	buffer := buf.StackNew()
	common.Must(EncodeRequestHeader(expectedRequest, expectedAddons, &buffer))

	Validator := new(vless.Validator)
	Validator.Add(user)

	actualRequest, actualAddons, err := DecodeRequestHeader(Validator, &buffer)
	common.Must(err)
...
func TestInvalidRequest(t *testing.T) {
	...
	_, _, err := DecodeRequestHeader(Validator, &buffer)
	if err == nil {
		t.Error("nil error")
	}
}
