...
/*
type BlakeSizeParser struct {
	blake  *blake3.Hasher
	buffer [2]byte
}

func NewBlakeSizeParser(nonce []byte) *BlakeSizeParser {
	blake := blake3.New()
	common.Must2(blake.Write(nonce))
	return &BlakeSizeParser{
		blake: blake,
	}
}

func (*BlakeSizeParser) SizeBytes() int32 {
	return 2
}

func (s *BlakeSizeParser) next() uint16 {
	common.Must2(s.blake.Digest().Read((s.buffer[:])))
	return binary.BigEndian.Uint16(s.buffer[:])
}

func (s *BlakeSizeParser) Decode(b []byte) (uint16, error) {
	mask := s.next()
	size := binary.BigEndian.Uint16(b)
	return mask ^ size, nil
}

func (s *BlakeSizeParser) Encode(size uint16, b []byte) []byte {
	mask := s.next()
	binary.BigEndian.PutUint16(b, mask^size)
	return b[:2]
}

func (s *BlakeSizeParser) NextPaddingLen() uint16 {
	return s.next() % 64
}

func (s *BlakeSizeParser) MaxPaddingLen() uint16 {
	return 64
}
*/
